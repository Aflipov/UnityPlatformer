...
public class PlayerMovement : MonoBehaviour
{
	...
	private string lasttAnimation = "Idle";
	...
	private void Move(float acceleration, float deceleration, Vector2 moveInput)
	{
		if (!_isDashing)
		{
			if (Mathf.Abs(moveInput.x) >= MoveStats.MoveThreshold)
			{
				...
				targetVelocity = moveInput.x * MoveStats.MaxMoveSpeed;
	...
	private void AnimationMoveCheck()
	{
		if (_isGrounded)
		{
		...
		else if (_rb.linearVelocityY > 0f)
		{
	...
	{
		if (lasttAnimation == currentAnimation)
		{
			return;
		}
		else
		{
			...
			lasttAnimation = currentAnimation;
		}
	}
